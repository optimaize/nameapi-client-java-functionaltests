...
/**
 */
public class EE_EmailNameParserCommandTest {
    ...
    public void test_EE_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "eliise.tamm@example.ee").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "eliise");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "tamm");
    }
    ...
    public void test_EE_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "eliisetamm@example.ee").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "eliise");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "tamm");
    }
