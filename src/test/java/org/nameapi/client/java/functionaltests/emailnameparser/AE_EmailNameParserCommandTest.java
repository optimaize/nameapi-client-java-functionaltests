...
/**
 */
public class AE_EmailNameParserCommandTest {
    ...
    public void test_AE_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "fatima.hussein@example.ae").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "fatima");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "hussein");
    }
    ...
    public void test_AE_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "fatimahussein@example.ae").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "fatima");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "hussein");
    }
