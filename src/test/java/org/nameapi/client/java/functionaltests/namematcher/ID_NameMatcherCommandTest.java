...
/**
 */
public class ID_NameMatcherCommandTest extends AbstractTest {
    ...
    public void test_ID_1() throws Exception {
        ...
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Mahyadi Panggabean")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("M. Panggabean")).build();
    ...
    public void test_ID_2() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Suparman Sukarno")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("S. Sukarno")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
