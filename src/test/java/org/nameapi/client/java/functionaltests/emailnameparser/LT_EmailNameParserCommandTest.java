...
/**
 */
public class LT_EmailNameParserCommandTest {
    ...
    public void test_LT_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "jokubas.paulauskas@example.lt").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "jokubas");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "paulauskas");
    }
    ...
    public void test_LT_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "jokubaspaulauskas@example.lt").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "jokubas");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "paulauskas");
    }
