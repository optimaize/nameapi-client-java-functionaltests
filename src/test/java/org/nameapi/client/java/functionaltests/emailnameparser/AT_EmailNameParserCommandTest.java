...
/**
 */
public class AT_EmailNameParserCommandTest {
    ...
    public void test_AT_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "klaus.lauda@example.at").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "klaus");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "lauda");
    }
    ...
    public void test_AT_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "klauslauda@example.at").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "klaus");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "lauda");
    }
