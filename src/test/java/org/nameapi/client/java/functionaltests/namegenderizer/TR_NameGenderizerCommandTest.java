...
/**
 */
public class TR_NameGenderizerCommandTest extends AbstractTest {
    ...
    public void test_TR_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Gülhan Yılmaz")).build();
    ...
    public void test_TR_2() throws Exception {
        PersonGenderizerCommand command = new PersonGenderizerCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Ayşe Öztürk")).build();
        GenderizerResult genderizerResult = executor.execute(command, mode, person).get();
