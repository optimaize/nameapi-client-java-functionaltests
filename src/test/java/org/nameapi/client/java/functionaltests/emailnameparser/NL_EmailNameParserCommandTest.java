...
/**
 */
public class NL_EmailNameParserCommandTest {

    private final CommandExecutor executor = NameApiRemoteExecutors.get();

    @Test
    public void test_NL_1() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "peter.vandersar@example.nl").get();
        ...
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "vandersar");
    }

    @Test
    public void test_NL_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "petervandersar@example.nl").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "peter");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "vandersar");
    }
