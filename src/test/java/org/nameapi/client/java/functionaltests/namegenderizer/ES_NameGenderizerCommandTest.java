...
/**
 */
public class ES_NameGenderizerCommandTest extends AbstractTest {
    ...
    public void test_ES_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("José Maria Fernández de la Torre")).build();
    ...
    public void test_ES_2() throws Exception {
        PersonGenderizerCommand command = new PersonGenderizerCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("María de los Ángeles López Sánchez")).build();
        GenderizerResult genderizerResult = executor.execute(command, mode, person).get();
