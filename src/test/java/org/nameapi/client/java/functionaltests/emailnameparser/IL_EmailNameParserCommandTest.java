...
/**
 */
public class IL_EmailNameParserCommandTest {
    ...
    public void test_IL_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "noam.levi@example.il").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "noam");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "levi");
    }

    @Test
    public void test_IL_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "noamlevi@example.il").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "noam");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "levi");
    }
