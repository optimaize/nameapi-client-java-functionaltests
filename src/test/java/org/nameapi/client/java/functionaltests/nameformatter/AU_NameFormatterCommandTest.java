...
/**
 */
public class AU_NameFormatterCommandTest extends AbstractTest {
    ...
    public void test_AU_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("aNaTjArI TjAkAmArRa")).build();
        ...
        assertEquals(formatterResult.getFormatted(), "Anatjari Tjakamarra");
    }

    @Test
    public void test_AU_2() throws Exception {
        PersonNameFormatterCommand command = new PersonNameFormatterCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();

        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("WAYNE BENTLEY PHILLIPS")).build();
        FormatterProperties properties = new FormatterProperties(true);
        PersonNameFormatterArgument argument = new PersonNameFormatterArgument(person, properties);
        FormatterResult formatterResult = executor.execute(command, mode, argument).get();
        assertEquals(formatterResult.getFormatted(), "Wayne Bentley Phillips");
    }
}
