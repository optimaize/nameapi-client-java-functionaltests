...
/**
 */
public class AU_EmailNameParserCommandTest {
    ...
    public void test_AU_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "wayne.phillips@example.au").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "wayne");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "phillips");
    }
    ...
    public void test_AU_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "waynephillips@example.au").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "wayne");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "phillips");
    }
