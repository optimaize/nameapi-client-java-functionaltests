...
/**
 */
public class TR_EmailNameParserCommandTest {
    ...
    public void test_TR_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "ayse.ozturk@example.tr").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "ayse");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "ozturk");
    }
}
