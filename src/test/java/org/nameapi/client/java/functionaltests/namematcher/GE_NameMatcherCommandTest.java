...
/**
 */
public class GE_NameMatcherCommandTest extends AbstractTest {

    private final CommandExecutor executor = NameApiRemoteExecutors.get();

    @Test
    public void test_GE_1() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Nikoloz Gelashvili")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("ნიკოლოზ გელაშვილი")).build();
    ...
    @Test
    public void test_GE_2() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Nikoloz Gelashvili")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Batoni N. Gelashvili")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
    ...
    public void test_GE_3() throws Exception {
        ...
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Nikoloz Gelashvili")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Nika Gelashvili")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
