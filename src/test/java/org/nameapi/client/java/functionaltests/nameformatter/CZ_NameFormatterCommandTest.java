...
/**
 */
public class CZ_NameFormatterCommandTest extends AbstractTest {
    ...
    public void test_CZ_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("rodina novákova")).build();
...
//    @Test
    public void test_CZ_2() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("kRiStÝnA \"TýNa\" nOvÁkOvA")).build();
        FormatterProperties properties = new FormatterProperties(true);
        PersonNameFormatterArgument argument = new PersonNameFormatterArgument(person, properties);
        FormatterResult formatterResult = executor.execute(command, mode, argument).get();
        assertEquals(formatterResult.getFormatted(), "Kristýna \"Týna\" Novákova");
    }
}
