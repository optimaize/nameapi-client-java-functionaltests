...
/**
 */
public class HU_EmailNameParserCommandTest {
    ...
    public void test_HU_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "szabo.levente@example.hu").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "levente");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "szabo");
    }
}
