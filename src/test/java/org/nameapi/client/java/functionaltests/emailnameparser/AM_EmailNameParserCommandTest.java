...
/**
 */
public class AM_EmailNameParserCommandTest {
    ...
    public void test_AM_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "anahit.hovhannisyan@example.am").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "anahit");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "hovhannisyan");
    }
    ...
    public void test_AM_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "anahithovhannisyan@example.am").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "anahit");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "hovhannisyan");
    }
