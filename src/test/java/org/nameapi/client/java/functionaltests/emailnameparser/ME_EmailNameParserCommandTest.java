...
/**
 */
public class ME_EmailNameParserCommandTest {
    ...
    public void test_ME_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "milica.medojevic@example.me").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "milica");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "medojevic");
    }
}
