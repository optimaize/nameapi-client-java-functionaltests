...
/**
 */
public class SE_EmailNameParserCommandTest {
    ...
    public void test_SE_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "christina.wikstrom@example.no").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "christina");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "wikstrom");
    }
    ...
    public void test_SE_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "christinawikstrom@example.no").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "christina");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "wikstrom");
    }
