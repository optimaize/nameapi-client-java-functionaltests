...
/**
 */
public class TM_EmailNameParserCommandTest {
    ...
    public void test_TM_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "balysh.owezow@example.tm").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "balysh");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "owezow");
    }

    @Test
    public void test_TM_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "balyshowezow@example.tm").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "balysh");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "owezow");
    }
