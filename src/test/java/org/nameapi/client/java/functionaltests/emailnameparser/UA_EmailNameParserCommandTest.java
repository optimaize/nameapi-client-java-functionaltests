...
/**
 */
public class UA_EmailNameParserCommandTest {
    ...
    public void test_UA_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "shevchenko@example.ua").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "shevchenko");
    }
}
