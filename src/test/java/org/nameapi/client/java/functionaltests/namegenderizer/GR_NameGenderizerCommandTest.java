...
/**
 */
public class GR_NameGenderizerCommandTest extends AbstractTest {
    ...
    public void test_GR_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Στυλιανος Αλέξανδρος Παπαδόπουλος")).build();
    ...
    public void test_GR_2() throws Exception {
        PersonGenderizerCommand command = new PersonGenderizerCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Stylianos Alexandros Papadopoulos")).build();
        GenderizerResult genderizerResult = executor.execute(command, mode, person).get();
        assertEquals(genderizerResult.getGender(), ComputedPersonGender.MALE);
    }
}
