...
/**
 */
public class UA_NameFormatterCommandTest extends AbstractTest {
    ...
    public void test_UA_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("АНДРІЙ МИКОЛАЙОВИЧ ШЕВЧЕНКО")).build();
        ...
        assertEquals(formatterResult.getFormatted(), "Андрій Миколайович Шевченко");
    }
    ...
    public void test_UA_2() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("aNdRiY MyKoLaYoVyCh sHeVcHeNkO")).build();
        FormatterProperties properties = new FormatterProperties(true);
        PersonNameFormatterArgument argument = new PersonNameFormatterArgument(person, properties);
        FormatterResult formatterResult = executor.execute(command, mode, argument).get();
        assertEquals(formatterResult.getFormatted(), "Andriy Mykolayovych Shevchenko");
    }
    ...
    @Test
    public void test_UA_4() throws Exception {
        PersonNameFormatterCommand command = new PersonNameFormatterCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
