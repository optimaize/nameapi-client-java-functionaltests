...
/**
 */
public class PH_EmailNameParserCommandTest {

    private final CommandExecutor executor = NameApiRemoteExecutors.get();

    @Test
    public void test_PH_1() throws Exception {
    ...
    @Test
    public void test_PH_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "dodongjulagay@example.ph").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "dodong");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "julagay");
    }
