...
/**
 */
public class CZ_EmailNameParserCommandTest {
    ...
    public void test_CZ_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "kristyna.novakova@example.cz").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "kristyna");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "novakova");
    }
    ...
    public void test_CZ_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "kristynanovakova@example.cz").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "kristyna");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "novakova");
    }
