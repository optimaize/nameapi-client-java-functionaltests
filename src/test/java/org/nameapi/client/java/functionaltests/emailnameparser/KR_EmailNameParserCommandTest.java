...
/**
 */
public class KR_EmailNameParserCommandTest {
    ...
    public void test_KR_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "pak.dongsu@example.kr").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "dongsu");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "pak");
    }
}
