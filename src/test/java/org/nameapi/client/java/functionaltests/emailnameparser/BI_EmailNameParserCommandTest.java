...
/**
 */
public class BI_EmailNameParserCommandTest {
    ...
    public void test_BI_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "veronique.nzeyimana@example.bi").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "veronique");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "nzeyimana");
    }
    ...
    public void test_BI_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "veroniquenzeyimana@example.bi").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "veronique");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "nzeyimana");
    }
