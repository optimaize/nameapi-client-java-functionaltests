...
/**
 */
public class BR_EmailNameParserCommandTest {
    ...
    public void test_BR_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "manuel.pereira@example.br").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "manuel");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "pereira");
    }
    ...
    public void test_BR_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "manuelpereira@example.br").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "manuel");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "pereira");
    }
