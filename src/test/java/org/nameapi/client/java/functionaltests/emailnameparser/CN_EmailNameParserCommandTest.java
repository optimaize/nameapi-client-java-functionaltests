...
/**
 */
public class CN_EmailNameParserCommandTest {
    ...
    public void test_CN_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "lee.donghai@example.cn").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "donghai");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "lee");
    }
