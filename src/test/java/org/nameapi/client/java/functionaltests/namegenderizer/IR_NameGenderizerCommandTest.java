...
/**
 */
public class IR_NameGenderizerCommandTest extends AbstractTest {
    ...
    public void test_IR_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("انوشه فاطمة مدنى")).build();
    ...
    public void test_IR_2() throws Exception {
        PersonGenderizerCommand command = new PersonGenderizerCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Anousheh Fatimah Madani")).build();
        GenderizerResult genderizerResult = executor.execute(command, mode, person).get();
        assertEquals(genderizerResult.getGender(), ComputedPersonGender.FEMALE);
    }
}
