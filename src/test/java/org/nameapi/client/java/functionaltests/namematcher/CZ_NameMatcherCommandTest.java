...
/**
 */
public class CZ_NameMatcherCommandTest extends AbstractTest {

    private final CommandExecutor executor = NameApiRemoteExecutors.get();

    @Test
    public void test_CZ_1() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Kristýna Nováková")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Kikina Nováková")).build();
    ...
    public void test_CZ_2() throws Exception {
        ...
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Kristýna Nováková")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Slečna Kikina Nováková")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
    ...
    @Test
    public void test_CZ_3() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Kristýna Nováková")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Kristýna \"Týna\" Novákova")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
