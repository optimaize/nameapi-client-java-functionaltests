...
/**
 */
public class TH_EmailNameParserCommandTest {
    ...
    public void test_TH_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "somchai.tinsulanonda@example.th").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "somchai");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "tinsulanonda");
    }
    ...
    public void test_TH_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "somchaitinsulanonda@example.th").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "somchai");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "tinsulanonda");
    }
