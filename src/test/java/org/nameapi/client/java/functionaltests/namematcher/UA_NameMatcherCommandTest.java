...
/**
 */
public class UA_NameMatcherCommandTest extends AbstractTest {

    private final CommandExecutor executor = NameApiRemoteExecutors.get();

    @Test
    public void test_UA_1() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Андрій Миколайович Шевченко")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Андрій Шевченко")).build();
    ...
    @Test
    public void test_UA_2() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Andriy Mykolayovych Shevchenko")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Andriy Shevchenko")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
    ...
    @Test
    public void test_UA_3() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Andriy Mykolayovych Shevchenko")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Андрій Миколайович Шевченко")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
