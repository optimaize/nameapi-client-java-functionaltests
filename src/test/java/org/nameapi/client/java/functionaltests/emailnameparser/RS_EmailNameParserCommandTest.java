...
/**
 */
public class RS_EmailNameParserCommandTest {
    ...
    public void test_RS_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "vladimir.velickovic@example.rs").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "vladimir");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "velickovic");
    }
    @Test
    public void test_RS_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "vladimirvelickovic@example.rs").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "vladimir");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "velickovic");
    }
