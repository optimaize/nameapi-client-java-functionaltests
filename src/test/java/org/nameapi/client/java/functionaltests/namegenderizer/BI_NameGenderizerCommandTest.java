...
/**
 */
public class BI_NameGenderizerCommandTest extends AbstractTest {
    ...
    public void test_BI_1() throws Exception {
        ...
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Véronique Nzeyimana")).build();
    ...
    public void test_BI_2() throws Exception {
        PersonGenderizerCommand command = new PersonGenderizerCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person = new NaturalInputPersonBuilder().name(makeName("Véronique \"Vero\" Nzeyimana")).build();
        GenderizerResult genderizerResult = executor.execute(command, mode, person).get();
        assertEquals(genderizerResult.getGender(), ComputedPersonGender.FEMALE);
    }
}
