...
/**
 */
public class LI_EmailNameParserCommandTest {
    ...
    public void test_LI_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "ursula.konzett@example.li").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "ursula");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "konzett");
    }
    ...
    public void test_LI_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "ursulakonzett@example.li").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "ursula");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "konzett");
    }
