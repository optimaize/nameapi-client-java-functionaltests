...
/**
 */
public class BE_EmailNameParserCommandTest {
    ...
    public void test_BE_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "francois.weyergans@example.be").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "francois");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "weyergans");
    }
    ...
    public void test_BE_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "francoisweyergans@example.be").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "francois");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "weyergans");
    }
