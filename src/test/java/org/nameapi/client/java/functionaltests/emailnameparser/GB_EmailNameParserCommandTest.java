...
/**
 */
public class GB_EmailNameParserCommandTest {
    ...
    public void test_GB_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "ethan.oneil@example.co.uk").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "ethan");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "oneil");
    }
