...
/**
 */
public class RS_NameMatcherCommandTest extends AbstractTest {
    ...
    public void test_RS_1() throws Exception {
        ...
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Vladimir Veličković")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Владимир Величковић")).build();
    ...
    public void test_RS_2() throws Exception {
        ...
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("Vladimir Veličković")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Vanja Veličković")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
    ...
    public void test_RS_3() throws Exception {
        PersonMatcherCommand command = new PersonMatcherCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        NaturalInputPerson person1 = new NaturalInputPersonBuilder().name(makeName("фамилија Јовановић")).build();
        NaturalInputPerson person2 = new NaturalInputPersonBuilder().name(makeName("Familija Jovanović")).build();
        PersonMatcherArgument argument = new PersonMatcherArgument(person1, person2);
        PersonMatcherResult result = executor.execute(command, mode, argument).get();
        assertEquals(result.getMatchType(), PersonMatchType.MATCHING);
    }
