...
/**
 */
public class IT_EmailNameParserCommandTest {
    ...
    public void test_IT_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "andrea.rossi@example.it").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "andrea");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "rossi");
    }
    ...
    public void test_IT_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "andrearossi@example.it").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "andrea");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "rossi");
    }
