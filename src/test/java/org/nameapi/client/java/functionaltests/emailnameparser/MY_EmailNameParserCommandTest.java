...
/**
 */
public class MY_EmailNameParserCommandTest {
    ...
    public void test_MY_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "r.anbuselvan@example.my").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "r.");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "anbuselvan");
    }
}
