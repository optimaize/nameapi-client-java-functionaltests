...
/**
 */
public class PT_EmailNameParserCommandTest {
    ...
    public void test_PT_1() throws Exception {
        ...
        EmailNameParserResult result = executor.execute(command, mode, "paulo.fernandes@example.pt").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "paulo");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "fernandes");
    }
    @Test
    public void test_PT_2() throws Exception {
        EmailNameParserCommand command = new EmailNameParserCommand();
        Mode mode = FunctionalTestsNameApiModeFactory.functionalTest();
        EmailNameParserResult result = executor.execute(command, mode, "paulofernandes@example.pt").get();
        assertEquals(result.getResultType(), EmailAddressParsingResultType.PERSON_NAME);
        assertEquals(result.getBestNameMatch().get().getGivenNames().get(0).getName(), "paulo");
        assertEquals(result.getBestNameMatch().get().getSurnames().get(0).getName(), "fernandes");
    }
